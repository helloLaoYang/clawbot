...
export type RouteAuthFixture = Readonly<{
  adminBearer: string
  adminPath: string
  botBearer: string
  botPath: string
  readServerOutput: () => string
  request: (path: string, headers: Readonly<Record<string, string>>) => Promise<HttpResponse>
  sessionCookie: string
}>

export type RouteAuthFixtureAccessor = () => RouteAuthFixture
...
export function useRouteAuthFixture(): RouteAuthFixtureAccessor {
  beforeAll(async () => {
    ...
    fixture = {
      ...
      botBearer: BOT_BEARER,
      botPath: `/api/v1/bots/${bot.publicId}/messages`,
      process: nextProcess,
      readServerOutput: () => serverOutput,
      request: (path, headers) => sendRequest(new URL(path, baseUrl), headers),
